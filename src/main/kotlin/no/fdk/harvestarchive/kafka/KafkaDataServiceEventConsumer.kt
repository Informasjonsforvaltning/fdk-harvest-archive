...
class KafkaDataServiceEventConsumer(
    @Qualifier("kafkaDataServiceEventCircuitBreaker")
    circuitBreaker: KafkaCircuitBreakerApi,
    archiveMetrics: ArchiveMetrics,
) {
    private val handler = KafkaHarvestEventHandler(circuitBreaker, archiveMetrics, ArchiveType.DATA_SERVICE)

    private fun logger(): Logger = LOGGER

    @KafkaListener(
        topics = [ArchiveType.TOPIC_DATA_SERVICE],
        groupId = "fdk-harvest-archive",
        containerFactory = "kafkaListenerContainerFactory",
        id = ArchiveType.LISTENER_DATA_SERVICE,
    )
