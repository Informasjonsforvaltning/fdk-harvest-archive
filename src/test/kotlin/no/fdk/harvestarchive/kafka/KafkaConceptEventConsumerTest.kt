...
class KafkaConceptEventConsumerTest {
    ...
    fun `consumeConceptEvent delegates record to circuit breaker and acknowledges`() {
        ...
        every { circuitBreaker.process(any()) } returns ProcessOutcome.Saved(no.fdk.harvestarchive.archive.ArchiveType.CONCEPT)
    ...
    @Test
    fun `consumeConceptEvent nacks on circuit breaker open`() {
        val record: ConsumerRecord<String, Any> = ConsumerRecord("concept-events", 0, 0L, "key", "any")
